...
class ActionPredict(object):
    ...
    def train(self, data_train,
        ...
        if test_train_model:
            return "/home/tianyao/code/ty/PAA/std/PedestrianActionBenchmark-main_ty/data/models/pie/PCPA/11Mar2025-21h28m43s", train_model
    ...
    # Test Functions
    def test(self, data_test, train_model, model_path=''):
        """
...
class PCPA(ActionPredict):
    ...
    def get_model(self, data_params):
        ...
        if len(encoder_outputs) > 1:
            att_enc_out = []
            x = Lambda(lambda x: K.expand_dims(x, axis=1), output_shape=(1, 256))(encoder_outputs[0])   # TY
            ...
            for i, enc_out in enumerate(encoder_outputs[1:]):
                ...
                x = Dropout(0.5)(x)
                x = Lambda(lambda x: K.expand_dims(x, axis=1), output_shape=(1, 256))(x)
