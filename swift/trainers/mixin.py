...
class SwiftMixin:
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        try:
            from safetensors.torch import save_file
            extra_state = {}
            # targets: 模块名的最后一段（例如 "visual.pan_patch_embed" -> "pan_patch_embed"）
            targets = {"ped_patch_embed", "classifier_head", "pre_bbox_head", "obs_bbox_head"}

            # unwrap model if wrapped by DDP/Accelerate/etc.
            try:
                _unwrap_model = unwrap_model(self.model)
            except Exception:
                _unwrap_model = getattr(self.model, "module", self.model)

            # Traverse named_modules to find modules with target base name
            for full_mod_name, module in _unwrap_model.named_modules():
                if not full_mod_name:  # skip top-level empty name unless it matches target
                    base = ""
                else:
                    base = full_mod_name.split(".")[-1]
                if base in targets:
                    # collect state dict with keys prefixed by full module name
                    for k, v in module.state_dict().items():
                        key = f"{full_mod_name}.{k}" if full_mod_name else f"{base}.{k}"
                        # ensure tensor moved to cpu for safe serialization
                        extra_state[key] = v.detach().cpu()

            if extra_state:
                save_path = os.path.join(output_dir, "extra_layer.safetensors")
                save_file(extra_state, save_path)
                logger.info(f"Saved extra layers to {save_path}")
            else:
                logger.warning("No extra layers found.")

        except Exception as e:
            logger.error(f"Failed to save extra layers: {e}")
