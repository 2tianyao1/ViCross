...
from .utils import (TEMPLATE_MAPPING, LazyLLMDataset, PtArguments, RLHFArguments, SftArguments, Template, dataset_map,
                    deep_getattr, dynamic_vit_gradient_checkpointing, get_mllm_arch, get_model_tokenizer,
                    get_template, get_time_info, print_example, set_generation_config, sort_by_max_length, stat_dataset)
...
def prepare_model_template_train(args, msg: Optional[Dict[str, Any]] = None):
    ...
    pan_resolution_h = args.pan_resolution_h
    kwargs['pan_resolution_h'] = pan_resolution_h
    pan_resolution_w = args.pan_resolution_w
    kwargs['pan_resolution_w'] = pan_resolution_w
    pan_patch_size = args.pan_patch_size
    kwargs['pan_patch_size'] = pan_patch_size
    ped_resolution_h = args.ped_resolution_h
    kwargs['ped_resolution_h'] = ped_resolution_h
    ped_resolution_w = args.ped_resolution_w
    kwargs['ped_resolution_w'] = ped_resolution_w
    ped_patch_size = args.ped_patch_size
    kwargs['ped_patch_size'] = ped_patch_size

    lossweight_class = args.lossweight_class
    kwargs['lossweight_class'] = lossweight_class
    lossweight_pre = args.lossweight_pre
    kwargs['lossweight_pre'] = lossweight_pre
    lossweight_obs = args.lossweight_obs
    kwargs['lossweight_obs'] = lossweight_obs
    ...
    for name, param in model.named_parameters():
        if "pan_patch_embed" in name or "ped_patch_embed" in name or "classifier_head" in name or "pre_bbox_head" in name or "obs_bbox_head" in name:
...
def prepare_dataset(args, template: Template, msg: Optional[Dict[str, Any]] = None):
    ...
    raw_data_train = imdb.generate_data_trajectory_sequence('train', **configs['data_opts'])
    raw_data_test = imdb.generate_data_trajectory_sequence('test', **configs['data_opts'])
    # raw_data_val = imdb.generate_data_trajectory_sequence('val', **configs['data_opts'])

    # raw_data_small_data = {key: value[89:94] for key, value in raw_data_train.items()}
    # raw_data_small_data["image_dimension"] = (1920, 1080)

    train_dataset = ActionDataset(args, 'train',  raw_data_train, configs, dataset = configs['model_opts']['dataset'])
    val_dataset = ActionDataset(args, 'test', raw_data_test, configs, dataset = configs['model_opts']['dataset'])
    # dataset_small_data = ActionDataset(args,'train',raw_data_small_data, configs, dataset)

    # frame_image_path_train = "/data1/ty/Qwen/data/bf_toy"
    # train_dataset = ActionDataset('train', frame_image_path_train)
    # val_dataset = ActionDataset('val', frame_image_path_train)
    if use_torchacc():
    ...
    if args.packing:
        ...
        if train_dataset is None:
            ...
            raise AttributeError('Failed to access dataset attributes,train_dataset is None. This might be because:\n'
                                 ...
                                 "(2) The 'max_length' setting is too short causing data_1 truncation.")
        if not args.streaming:
            dataset_info['train_dataset'] = stat_dataset(train_dataset)
            if val_dataset is not None:
                dataset_info['val_dataset'] = stat_dataset(val_dataset)
    else:
        td0, tkwargs0 = template.encode(train_dataset[1])
        print_example(td0, tokenizer, tkwargs0)
        train_dataset = LazyLLMDataset(train_dataset, template.encode)
        if val_dataset is not None:
            val_dataset = LazyLLMDataset(val_dataset, template.encode)
...
def llm_sft(args: SftArguments) -> Dict[str, Any]:
    ...
    seed_everything(args.seed, full_determinism = True)
