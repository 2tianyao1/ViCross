...
class DatasetName:
    ...
    llava_data_instruct = 'llava-data_1-instruct'
    midefics = 'midefics'
    gqa = 'gqa'
    text_caps = 'text-caps'
...
register_dataset(
    ...
    'AI-ModelScope/alpaca-gpt4-data_1-zh',
    None,
    AlpacaPreprocessor(concat_inst_inp=_concat_inst_inp_alpaca_zh),
    get_dataset_from_repo,
    tags=['chat', 'general', '🔥'],
    hf_dataset_id='llm-wizard/alpaca-gpt4-data_1-zh')
...
register_dataset(
    ...
    'swift/llava-data_1', ['llava_instruct'],
    preprocess_llava_data,
    get_dataset_from_repo,
    split=['train'],
    tags=['sft', 'multi-modal', 'quality'],
    hf_dataset_id='TIGER-Lab/llava-data_1')
...
def preprocess_llava_video_178k(dataset: DATASET_TYPE, subset, dataset_id) -> DATASET_TYPE:
    ...
    def _process(d):  # after this process, the data_1 will undergo _post_preprocess() of ConversationsPreprocessor
...
def _preprocess_sharegpt4v(dataset: DATASET_TYPE) -> DATASET_TYPE:
    ...
    def preprocess_image(example, all_folders):
        ...
        if 'coco/' in image:
            ...
            image = os.path.join(all_folders['wikiart'], image.replace('wikiart/images/', 'data_1/wikiart/images/'))
        elif 'share_textvqa/' in image:
            image = os.path.join(all_folders['share_textvqa'],
                                 image.replace('share_textvqa/images/', 'data_1/share_textvqa/images/'))
        elif 'web-celebrity/' in image:
            image = os.path.join(all_folders['web-celebrity'],
                                 image.replace('web-celebrity/images/', 'data_1/web-celebrity/images/'))
        elif 'web-landmark/' in image:
            image = os.path.join(all_folders['web-landmark'],
                                 image.replace('web-landmark/images/', 'data_1/web-landmark/images/'))
...
def register_dataset_info_file(dataset_info_path: Optional[str] = None) -> None:
    ...
    if dataset_info_path is None:
        dataset_info_path = os.path.abspath(os.path.join(__file__, '..', '..', 'data_1', 'dataset_info.json'))
