...
class Template:
    ...
    def _preprocess_media(self, example):
        ...
        crop_ped_images = example.get('crop_ped_images') or []
        if images:
            if example.get('objects') or self.load_medias or self._is_lmdeploy or self._is_vllm:
                ...
                crop_ped_images = load_batch(crop_ped_images, load_image)
            ...
            example['crop_ped_images'] = crop_ped_images
    ...
    def data_collator(self, batch: List[Dict[str, Any]], padding_to: Optional[int] = None) -> Dict[str, Any]:
        """
        Args:
            batch(`List[Dict[str, Any]]`): The input data_1 in batch
        ...
        # TY
        if 'past_bbox' in batch[0]:
            past_bbox = [b['past_bbox'] for b in batch]
            past_bbox = torch.tensor(past_bbox, device=res['inputs_embeds'].device)
            res['past_bbox'] = past_bbox
        if 'crossing_point_bbox' in batch[0]:
            crossing_point_bbox = [b['crossing_point_bbox'] for b in batch]
            crossing_point_bbox = torch.tensor(crossing_point_bbox, device=res['inputs_embeds'].device)
            res['crossing_point_bbox'] = crossing_point_bbox
        if 'image_mask' in batch[0]:
            image_mask = torch.stack([b['image_mask'].squeeze(0) for b in batch], dim=0)  # (batch_size, seq, dim)
            res['image_mask'] = image_mask
        return res
...
class _Qwen2VLTemplateMixin:
    ...
    def _encode(self, example: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        crop_ped_images = example.get('crop_ped_images')
        data = {}
        data['new_bbox'] = example.get('new_bbox')
        data['crossing_point_bbox'] = example.get('crossing_point_bbox')
        data['past_bbox'] = example.get('past_bbox')
        for media_type in ['images', 'videos']:
            if locals()[media_type]:
                if media_type == 'images':
                    ...
                    # TY
                    crop_media_inputs = processor.image_processor(
                        images=crop_ped_images, videos=None, return_tensors='pt', do_resize=False, patch_size=7)
                    crop_image_grid_thw = crop_media_inputs['image_grid_thw']
                    crop_pixel_values = crop_media_inputs['pixel_values']
                    data['crop_image_grid_thw'] = crop_image_grid_thw
                    data['crop_pixel_values'] = crop_pixel_values
    ...
    def _post_encode(self, model, data: Any) -> Dict[str, Any]:
        ...
        crossing_point_bbox = data.get('crossing_point_bbox')
        past_bbox = data.get('past_bbox')
        inputs_embeds = _model.embed_tokens(input_ids)

        # TY
        new_bbox = data['new_bbox']
        crop_pixel_values = data.get('crop_pixel_values')
        crop_image_grid_thw = data.get('crop_image_grid_thw')
        ...
        if pixel_values is None and pixel_values_videos is None:  # plain-text
            ...
            if pixel_values is not None:
                ...
                pan_pixel_values = pixel_values.type(model.visual.get_dtype())
                new_bbox = torch.tensor(new_bbox, device=pan_pixel_values.device)
                crop_pixel_values = crop_pixel_values.type(model.visual.get_dtype())
                # image_embeds = model.visual(pixel_values, grid_thw=image_grid_thw)
                image_embeds = model.visual(hidden_states=pan_pixel_values, grid_thw=image_grid_thw,
                                            crop_hidden_states=crop_pixel_values, new_bbox=new_bbox)
        ...
        return {'inputs_embeds': inputs_embeds[0], 'image_mask': image_mask, 'crossing_point_bbox':crossing_point_bbox, 'past_bbox': past_bbox}
...
DBRX_SYSTEM = (
    ...
    'You do not have real-time data_1 access or code execution capabilities.'
    ' You avoid stereotyping and provide balanced perspectives on controversial topics. '
    'You do not provide song lyrics, poems, or news articles and do not divulge details of your training data_1.\n'
