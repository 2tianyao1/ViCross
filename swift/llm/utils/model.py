...
# ty
from transformers import AutoTokenizer, Qwen2VLForConditionalGeneration, Qwen2VLModel
from transformers.models.qwen2_vl.modeling_qwen2_vl import Qwen2VisionTransformerPretrainedModel, PatchEmbed
import torch.nn.functional as F
from torch import nn
from  torch.cuda.amp import autocast
from ty.util.util import patches_union_pixel_bbox_from_bbox
...
def fuse_pan_and_crop(pan_hidden_states, crop_hidden_states, bboxes,
                      pan_patch_size, ped_patch_size,
                      ped_grid_h, ped_grid_w, pan_grid_h, pan_grid_w,
                       eps=1e-6):
    """
    两阶段融合：先几何映射（patch -> coarse cell），再在全图尺度做 content-aware 融合。

    Args:
        pan_hidden_states:  (B, 16*4*8, C)    # coarse tokens flattened
        crop_hidden_states: (B, 16*16*16, C)  # fine tokens flattened
        bboxes:             (B, 16, 4)        # x0,y0,x1,y1 (在 resized 背景图上的坐标，例如 224x126)
        use_learnable_att:  bool               # 是否使用可学习的 conv 来产生 att_map（推荐训练时为 True 并传入 att_conv）
        att_conv:           nn.Conv2d or None  # 可选：外部传入的 conv (C -> 1)
        alpha:              float              # coarse vs fine 融合权重 (0..1)，1 表示只保留 coarse
        eps:                float              # 防止除零
    """
    B, _, C = pan_hidden_states.shape
    H_pan, W_pan = pan_grid_h, pan_grid_w
    H_crop, W_crop = ped_grid_h, ped_grid_w
    P_size = pan_patch_size
    C_size = ped_patch_size

    # reshape 为 [B, T=16, H, W, C]
    pan = pan_hidden_states.view(B, 16, H_pan, W_pan, C)
    crop = crop_hidden_states.view(B, 16, H_crop, W_crop, C)

    device = pan.device

    # --- 阶段 1: 几何映射（patch-level alignment） ---
    # pooled_fine 的形状 (B, 16, H_pan, W_pan, C)，初始化为 pan 的 clone（以便 later 融合时对未覆盖 cell 保持原值）
    pooled_fine = pan.clone() * 0.0  # 先置 0，下面会填入在 bbox 所覆盖区域的聚合结果
    coverage_mask = torch.zeros(B, 16, H_pan, W_pan, dtype=torch.bool, device=device)

    for b in range(B):
        for t in range(16):
            x0, y0, x1, y1 = bboxes[b, t].tolist()
            # 有效性检查
            if x1 <= x0 or y1 <= y0:
                continue

            coarse_embeds = pan[b, t]   # (H_pan, W_pan, C)
            fine_embeds = crop[b, t]    # (H_crop, W_crop, C)
            Hc, Wc, D = coarse_embeds.shape
            Hf, Wf, _ = fine_embeds.shape

            # coarse cell 范围（使用与之前一致的映射策略）
            c0 = int(x0 // P_size)
            c1 = int((x1 - 1) // P_size)
            r0 = int(y0 // P_size)
            r1 = int((y1 - 1) // P_size)
            ...
            coarse_b = bboxes[b, t]
            patches_union_pixel_bbox = patches_union_pixel_bbox_from_bbox(coarse_b,W = W_pan*pan_patch_size, H=H_pan*pan_patch_size,P_size=pan_patch_size)
            x0, y0, x1, y1 = patches_union_pixel_bbox

            # 对每个被 bbox 覆盖的 coarse cell，找到其在 fine grid 上对应的 token 范围并平均
            for r in range(r0, r1 + 1):
                for c in range(c0, c1 + 1):
                    if r < 0 or r >= Hc or c < 0 or c >= Wc:
                        continue

                    # coarse cell 对应的背景像素范围
                    bx0, bx1 = c * P_size, (c + 1) * P_size
                    by0, by1 = r * P_size, (r + 1) * P_size

                    # 投影到行人 crop 图坐标 (假设 crop resize 为 112x112)
                    # 这里沿用你的投影公式（注意分母不会为零，因为前面做了 x1>x0, y1>y0 的检查）
                    rel_x0 = (bx0 - x0) / (x1 - x0) * 112.0
                    rel_x1 = (bx1 - x0) / (x1 - x0) * 112.0
                    rel_y0 = (by0 - y0) / (y1 - y0) * 112.0
                    rel_y1 = (by1 - y0) / (y1 - y0) * 112.0

                    # 转换为 fine grid 索引
                    f_c0 = int(rel_x0 // C_size)
                    f_c1 = int((rel_x1 - 1) // C_size)
                    f_r0 = int(rel_y0 // C_size)
                    f_r1 = int((rel_y1 - 1) // C_size)

                    # clamp
                    f_c0 = max(0, min(f_c0, Wf - 1))
                    f_c1 = max(0, min(f_c1, Wf - 1))
                    f_r0 = max(0, min(f_r0, Hf - 1))
                    f_r1 = max(0, min(f_r1, Hf - 1))

                    # 选取对应 fine tokens 并做平均（或将来可替换为加权）
                    selected = fine_embeds[f_r0:f_r1 + 1, f_c0:f_c1 + 1, :]  # (h, w, C)
                    if selected.numel() == 0:
                        continue
                    selected_flat = selected.reshape(-1, D)  # (n, C)
                    pooled_vec = selected_flat.mean(dim=0)   # (C,)

                    pooled_fine[b, t, r, c, :] = pooled_vec
                    coverage_mask[b, t, r, c] = True

    # --- 阶段 2: Content-aware 融合（基于 pooled_fine 的全图注意） ---
    # pooled_fine 现在是对齐后的细粒度信息（形状与 pan 一致）
    B_T = B * 16
    pooled_fine_flat = pooled_fine.view(B_T, H_pan, W_pan, C).permute(0, 3, 1, 2).contiguous()
    pan_flat = pan.view(B_T, H_pan, W_pan, C).permute(0, 3, 1, 2).contiguous()

    # 注意力计算

    energy = pooled_fine_flat.norm(dim=1, keepdim=True)
    att_map = torch.sigmoid(energy / (C ** 0.5 + eps))

    att_map_expand = att_map.expand(-1, C, -1, -1)

    # === 关键修改：仅在行人区域内融合 ===
    mask_flat = coverage_mask.view(B_T, H_pan, W_pan).unsqueeze(1)  # (B_T, 1, H, W)
    fused_flat = torch.where(
        mask_flat,  # 仅行人区域
        att_map_expand * pooled_fine_flat + 1 * pan_flat,
        pan_flat  # 非行人区域保持原样
    )
    # reshape 回原格式
    fused = fused_flat.permute(0, 2, 3, 1).contiguous().view(B, 16, H_pan, W_pan, C)
    return fused.view(B * 16 * H_pan * W_pan, C)

class MyVisionTransformer(Qwen2VisionTransformerPretrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        self.pan_resolution_h = kwargs.pop('pan_resolution_h', None)
        self.pan_resolution_w = kwargs.pop('pan_resolution_w', None)
        self.pan_patch_size = kwargs.pop('pan_patch_size', None)
        self.pan_grid_h = self.pan_resolution_h // self.pan_patch_size
        self.pan_grid_w = self.pan_resolution_w // self.pan_patch_size

        self.ped_resolution_h = kwargs.pop('ped_resolution_h', None)
        self.ped_resolution_w = kwargs.pop('ped_resolution_w', None)
        self.ped_patch_size = kwargs.pop('ped_patch_size', None)
        self.ped_grid_h = self.ped_resolution_h // self.ped_patch_size
        self.ped_grid_w = self.ped_resolution_w // self.ped_patch_size
        super().__init__(config, *inputs, **kwargs)

        self.ped_patch_embed = PatchEmbed(
            patch_size=self.ped_patch_size,
        ...
        self.pan_patch_embed = PatchEmbed(
            patch_size=self.pan_patch_size,
            temporal_patch_size=config.temporal_patch_size,
            in_channels=config.in_channels,
            embed_dim=config.embed_dim,
        )
        self.pan_patch_embed = self.pan_patch_embed.float()
        self.ped_patch_embed = self.ped_patch_embed.float()
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        crop_hidden_states = kwargs['crop_hidden_states']
        new_bbox = kwargs['new_bbox']

        if self.pan_patch_size == 14:
            pan_hidden_states = self.patch_embed(hidden_states)  # 14*14
        else:
            pan_hidden_states = self.pan_patch_embed(hidden_states)

        crop_hidden_states = self.ped_patch_embed(crop_hidden_states)
        pan_hidden_states = pan_hidden_states.view(
            pan_hidden_states.size(0) // (16 * self.pan_grid_w * self.pan_grid_h),
            16 * self.pan_grid_w * self.pan_grid_h, 1280)
        crop_hidden_states = crop_hidden_states.view(
            crop_hidden_states.size(0) // (16 * self.ped_grid_w * self.ped_grid_h),
            16 * self.ped_grid_w * self.ped_grid_h, 1280)
        new_bbox = new_bbox.unsqueeze(0)
        hidden_states = fuse_pan_and_crop(pan_hidden_states, crop_hidden_states, new_bbox, pan_patch_size = self.pan_patch_size, ped_patch_size = self.ped_patch_size, ped_grid_h = self.ped_grid_h, ped_grid_w=self.ped_grid_w, pan_grid_h = self.pan_grid_h, pan_grid_w=self.pan_grid_w)
...
class Qwen2VLForMultiTask(Qwen2VLForConditionalGeneration):
    def __init__(self, config, **model_kwargs):
        super().__init__(config)
        self.pan_resolution_h = model_kwargs.get('pan_resolution_h')
        self.pan_resolution_w = model_kwargs.get('pan_resolution_w')
        self.pan_patch_size = model_kwargs.get('pan_patch_size')
        self.ped_resolution_h = model_kwargs.get('ped_resolution_h')
        self.ped_resolution_w = model_kwargs.get('ped_resolution_w')
        self.ped_patch_size = model_kwargs.get('ped_patch_size')

        self.lossweight_class = model_kwargs.get('lossweight_class')
        self.lossweight_obs = model_kwargs.get('lossweight_obs')
        self.lossweight_pre = model_kwargs.get('lossweight_pre')

        self.visual = MyVisionTransformer(config.vision_config,
                                          pan_resolution_h=self.pan_resolution_h,
                                          pan_resolution_w=self.pan_resolution_w,
                                          pan_patch_size=self.pan_patch_size, ped_resolution_h=self.ped_resolution_h,
                                          ped_resolution_w=self.ped_resolution_w, ped_patch_size=self.ped_patch_size, )
        # multitask
        self.classifier_head = nn.Linear(config.hidden_size, 1)
        self.pre_bbox_head = nn.Linear(config.hidden_size, 4)
        self.obs_bbox_head = nn.Linear(config.hidden_size, 4)

        self.classifier_head = self.classifier_head.float()
        self.pre_bbox_head = self.pre_bbox_head.float()
        self.obs_bbox_head = self.obs_bbox_head.float()

        self.is_predict = model_kwargs.get('is_predict')
        self.is_classifier = model_kwargs.get('is_classifier')
        self.is_pre_bbox = model_kwargs.get('is_pre_bbox')
        self.is_obs_bbox = model_kwargs.get('is_obs_bbox')
        pos_weight = model_kwargs.get('pos_weight')
        pos_weight = torch.tensor(pos_weight)
        pos_weight = pos_weight.cuda()
        self.criterion_content = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        self.bbox_loss_fn = nn.SmoothL1Loss(beta=1.0, reduction='mean')
    ...
    def forward(
        ...
        crossing_point_bbox = kwargs.pop('crossing_point_bbox', None)
        past_bbox = kwargs.pop('past_bbox', None)
        image_mask = kwargs.pop('image_mask', None)

        outputs = super().forward(input_ids, attention_mask, position_ids, past_key_values, inputs_embeds,
                                  labels, use_cache, **kwargs)
        ...
        if crossing_point_bbox is not None:
            crossing_point_bbox = crossing_point_bbox.to(device)
        if past_bbox is not None:
            past_bbox = past_bbox.to(device)

        if not self.is_predict:
            hidden_states = outputs.hidden_states[-1]
            input_no_label = (labels == -100)
            hidden_states = torch.stack([
                hidden_states[i, mask, :] for i, mask in enumerate(input_no_label)
            ])
        else:
            hidden_states = outputs.hidden_states[-1]

        #for pre+cls
        hidden_states_cls_pre = hidden_states.mean(dim=1)
        # for obs
        if image_mask != None:
            image_mask = image_mask.any(dim=-1)
            image_mask = image_mask[:, :-3]
            vision_states = hidden_states[image_mask].view(hidden_states.size(0), -1,hidden_states.size(-1))  # (B, 16*32, D)
            vision_states = vision_states.view(hidden_states.size(0), 16, vision_states.size(1)//16 , hidden_states.size(-1)) # (B, 16, 32, D)
            hidden_states_vision = vision_states.mean(dim=2)  # (B, 16, D)

            with autocast():
                logits_obs_bbox = self.obs_bbox_head(hidden_states_vision)  # logits: [batch_size, 16, 4]

        with autocast():
            logits_cls = self.classifier_head(hidden_states_cls_pre)  # logits: [batch_size, num_classes]
            logits_pre_bbox = self.pre_bbox_head(hidden_states_cls_pre)  # logits: [batch_size, num_classes]
        if self.is_classifier:
            return logits_cls
        if self.is_pre_bbox:
            return logits_pre_bbox
        if self.is_obs_bbox:
            return logits_obs_bbox


        if not self.is_predict:
            # labels = labels[labels != -100]
            mask = (labels != -100)  # shape (4, 706)
            filtered_labels = [labels[i][mask[i]] for i in range(labels.size(0))]
            class_label = []
            crossing_tokens = [28842, 287, 151645]
            not_crossing_tokens = [1921, 26638, 151645]
            for label_token in filtered_labels:
                tensor_list = label_token.cpu().tolist()
                if tensor_list == crossing_tokens:
                    class_label.append(1)
                elif tensor_list == not_crossing_tokens:
                    class_label.append(0)
                else:
                    raise ValueError(f"Unexpected token sequence: {tensor_list}")
            class_label = torch.tensor(class_label)
            generate_loss = outputs.loss
            device = logits_cls.device
            class_label = class_label.float().to(device)

            class_loss = self.criterion_content(logits_cls.view(-1), class_label)
            pre_bbox_loss = self.bbox_loss_fn(logits_pre_bbox, crossing_point_bbox)
            obs_bbox_loss = self.bbox_loss_fn(logits_obs_bbox, past_bbox)

            # beta = 0.00000001
            # loss = (1 - beta) * generate_loss + (beta / 3) * class_loss + (beta / 3) * pre_bbox_loss + (beta / 3) * obs_bbox_loss
            loss = 1 * generate_loss + self.lossweight_class * class_loss + self.lossweight_pre * pre_bbox_loss + self.lossweight_obs * obs_bbox_loss
            outputs.loss = loss
    ...
    def from_pretrained(cls, model_dir, *args, **kwargs):
        model = super().from_pretrained(model_dir, *args, **kwargs)
        return model
...
def get_model_tokenizer_from_repo(model_dir: str,
    ...
    if load_model:
        if kwargs.get('use_unsloth', False):
            ...
            with context:
                model = Qwen2VLForMultiTask.from_pretrained(
...
def get_model_tokenizer(model_type: str,
                        ...
                        *,
                        ...
                        is_predict: Optional[bool] = False,
                        is_classifier: Optional[bool] = False,
                        is_pre_bbox: Optional[bool] = False,
                        is_obs_bbox: Optional[bool] = False,
                        pan_resolution_h: Optional[int] = None,
                        pan_resolution_w: Optional[int] = None,
                        pan_patch_size: Optional[int] = None,
                        ped_resolution_h: Optional[int] = None,
                        ped_resolution_w: Optional[int] = None,
                        ped_patch_size: Optional[int] = None,
                        lossweight_class: Optional[float] = None,
                        lossweight_pre: Optional[float] = None,
                        lossweight_obs: Optional[float] = None,
    ...
    if model_kwargs is None:
        model_kwargs = {}
    model_kwargs['pos_weight'] = pos_weight
    model_kwargs['is_predict'] = is_predict
    model_kwargs['is_classifier'] = is_classifier
    model_kwargs['is_pre_bbox'] = is_pre_bbox
    model_kwargs['is_obs_bbox'] = is_obs_bbox

    model_kwargs['pan_resolution_h'] = pan_resolution_h
    model_kwargs['pan_resolution_w'] = pan_resolution_w
    model_kwargs['pan_patch_size'] = pan_patch_size
    model_kwargs['ped_resolution_h'] = ped_resolution_h
    model_kwargs['ped_resolution_w'] = ped_resolution_w
    model_kwargs['ped_patch_size'] = ped_patch_size

    model_kwargs['lossweight_class'] = lossweight_class
    model_kwargs['lossweight_obs'] = lossweight_obs
    model_kwargs['lossweight_pre'] = lossweight_pre
