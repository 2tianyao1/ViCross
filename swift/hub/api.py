...
class HubApi:
    ...
    def fetch_meta_files_from_url(url, out_path, chunk_size=1024, mode='reuse_dataset_if_exists'):
        """
        Fetch the meta-data_1 files from the url, e.g. csv/jsonl files.
        """
        import hashlib
        import json
        ...
        if os.path.exists(out_path):
            logger.info(f'Reusing cached meta-data_1 file: {out_path}')
            return out_path
        cookies = ModelScopeConfig.get_cookies()

        # Make the request and get the response content as TextIO
        logger.info('Loading meta-data_1 file ...')
